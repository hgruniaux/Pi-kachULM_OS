...
static inline int to_chars(const floating_decimal_64 v, const bool sign, char* const result) {
  ...
  if ((output >> 32) != 0) {
    ...
    libk::memcpy(result + index + olength - 1, DIGIT_TABLE + c0, 2);
    libk::memcpy(result + index + olength - 3, DIGIT_TABLE + c1, 2);
    libk::memcpy(result + index + olength - 5, DIGIT_TABLE + d0, 2);
    libk::memcpy(result + index + olength - 7, DIGIT_TABLE + d1, 2);
    i += 8;
  }
  ...
  while (output2 >= 10000) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    libk::memcpy(result + index + olength - i - 1, DIGIT_TABLE + c0, 2);
    libk::memcpy(result + index + olength - i - 3, DIGIT_TABLE + c1, 2);
    i += 4;
  }
  if (output2 >= 100) {
    const uint32_t c = (output2 % 100) << 1;
    output2 /= 100;
    libk::memcpy(result + index + olength - i - 1, DIGIT_TABLE + c, 2);
    i += 2;
  }
  if (output2 >= 10) {
