...
static inline void append_n_digits(const uint32_t olength, uint32_t digits, char* const result) {
  ...
  while (digits >= 10000) {
    ...
    libk::memcpy(result + olength - i - 2, DIGIT_TABLE + c0, 2);
    libk::memcpy(result + olength - i - 4, DIGIT_TABLE + c1, 2);
    i += 4;
  }
  if (digits >= 100) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    libk::memcpy(result + olength - i - 2, DIGIT_TABLE + c, 2);
    i += 2;
  }
  if (digits >= 10) {
...
static inline void append_d_digits(const uint32_t olength, uint32_t digits, char* const result) {
  ...
  while (digits >= 10000) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    libk::memcpy(result + olength + 1 - i - 2, DIGIT_TABLE + c0, 2);
    libk::memcpy(result + olength + 1 - i - 4, DIGIT_TABLE + c1, 2);
    i += 4;
  }
  if (digits >= 100) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    libk::memcpy(result + olength + 1 - i - 2, DIGIT_TABLE + c, 2);
    i += 2;
  }
  if (digits >= 10) {
    const uint32_t c = digits << 1;
...
static inline void append_c_digits(const uint32_t count, uint32_t digits, char* const result) {
  ...
  for (; i < count - 1; i += 2) {
    const uint32_t c = (digits % 100) << 1;
    digits /= 100;
    libk::memcpy(result + count - i - 2, DIGIT_TABLE + c, 2);
  }
...
static inline void append_nine_digits(uint32_t digits, char* const result) {
  ...
  for (uint32_t i = 0; i < 5; i += 4) {
    ...
    const uint32_t c0 = (c % 100) << 1;
    const uint32_t c1 = (c / 100) << 1;
    libk::memcpy(result + 7 - i, DIGIT_TABLE + c0, 2);
    libk::memcpy(result + 5 - i, DIGIT_TABLE + c1, 2);
  }
  result[0] = (char)('0' + digits);
}
...
#endif
  libk::memcpy(result + sign, "Infinity", 8);
  return sign + 8;
}
...
int d2fixed_buffered_n(double d, uint32_t precision, char* result) {
  ...
  if (ieeeExponent == 0 && ieeeMantissa == 0) {
    ...
    if (precision > 0) {
      result[index++] = '.';
      libk::memset(result + index, '0', precision);
      index += precision;
    }
    return index;
  }
  ...
  if (e2 < 0) {
    ...
    if (blocks <= MIN_BLOCK_2[idx]) {
      ...
      i = MIN_BLOCK_2[idx];
      libk::memset(result + index, '0', 9 * i);
      index += 9 * i;
    }
    for (; i < blocks; ++i) {
      ...
      if (p >= POW10_OFFSET_2[idx + 1]) {
        ...
        libk::memset(result + index, '0', fill);
        index += fill;
        break;
      }
      // Temporary: j is usually around 128, and by shifting a bit, we push it to 128 or above, which is
...
int d2exp_buffered_n(double d, uint32_t precision, char* result) {
  ...
  if (ieeeExponent == 0 && ieeeMantissa == 0) {
    ...
    libk::memcpy(result + index, "e+00", 4);
    index += 4;
    return index;
  }
  ...
  if (printedDigits != 0) {
    if (digits == 0) {
      libk::memset(result + index, '0', maximum);
    } else {
      append_c_digits(maximum, digits, result + index);
    }
    index += maximum;
  ...
  if (exp >= 100) {
    ...
    libk::memcpy(result + index, DIGIT_TABLE + 2 * (exp / 10), 2);
    result[index + 2] = (char)('0' + c);
    index += 3;
  } else {
    libk::memcpy(result + index, DIGIT_TABLE + 2 * exp, 2);
    index += 2;
  }
